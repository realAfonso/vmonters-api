<?
	...
	function getBuddy($userId) {
		...
		$r = $db->select("vms_user_has_species", "WHERE user = '$userId' AND buddy = '1'");
		$uhs = mysqli_fetch_array($r, MYSQLI_ASSOC);

		if($uhs == null) return null;

		return getMonster($uhs["id"]);
	}

	function getMonster($monsterId) {
		$db = new Database();

		$r = $db->select("vms_active_monsters", "WHERE id = '$monsterId'");
		...
		$monster["steps"] = getStepsFromMonster($monsterId);
		...
		$monster[baseSpAtk] = $monster[spatk];
		$monster[baseSpDef] = $monster[spdef];
		$monster[baseSpd] = $monster[spd];

		$monster[hp] = $monster[hp] + intval($monster[euHp]/4);
		$monster[atk] = $monster[atk] + intval($monster[euAtk]/4);
		$monster[def] = $monster[def] + intval($monster[euDef]/4);
		$monster[spatk] = $monster[spatk] + intval($monster[euSpAtk]/4);
		$monster[spdef] = $monster[spdef] + intval($monster[euSpDef]/4);
		$monster[spd] = $monster[spd] + intval($monster[euSpd]/4);
