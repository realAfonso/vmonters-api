<?
	...
	$opponent = getUser($data[opponentId]);
	...
	if($mon1[hp]>0) $return[winner] = $mon1desc;
	else $return[winner] = $mon2desc;

	if($return[winner] == "CHALLENGER"){
		$return[bits] = intval($bits * 90 / 100);
	}else{
		$return[bits] = intval($bits * 10 / 100);
	}
